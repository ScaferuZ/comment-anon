...
export default function HandlePage() {
  ...
  const { data, refetch } = useQuery({
    queryFn: () =>
      fetch(`/api/comments?page=${handle}`).then((res) => res.json()) as Promise<Array<Comment>>,
      ...
      {!mutation.isLoading && !mutation.isSuccess && (
        <div className="w-full flex justify-center">
          <form className="w-full max-w-xs" onSubmit={handleSubmit}>
            <div className="mb-4">
              <label className="block text-gray-700 font0bold mb-2" htmlFor="handle">
                Handle
              </label>
              <input
                ...
                name="comment"
                type="text"
                placeholder="Ask someting here..."
              />
            </div>
            <div>
              <button className="bg-blue-500 rounded-xl shadow text-white py-2 px-4" type="submit">
                Ask something
              </button>
            </div>
          </form>
